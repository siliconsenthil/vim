" lodgeit.vim: Vim plugin for paste.pocoo.org MODIFIED TO USE paste.slicehost.com
...
srv = ServerProxy('https://gavri:;slashyears17@paste.slicehost.com/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^https://paste.slicehost.com/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = 'https://paste.slicehost.com/show/%s' % paste_id
